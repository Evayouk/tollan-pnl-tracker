export default async function handler(req, res) {
  ...
  try {
    if (action === 'rewards') {
      ...
      // Use Etherscan to get ALL transactions and filter deposits
      const url = `${ETHERSCAN}?chainid=2741&module=account&action=txlist&address=${address}&startblock=0&endblock=99999999&sort=desc&apikey=${APIKEY}`;
      const r = await fetch(url);
      const data = await r.json();
      const results = [];
      for (const tx of (data.result || [])) {
        if (tx.from?.toLowerCase() === address.toLowerCase() && parseInt(tx.value) > 0) {
          // Check internal txs to see if deposit contract was called
          results.push({ hash: tx.hash, value: parseInt(tx.value) / 1e18, to: tx.to, timeStamp: tx.timeStamp });
        }
      }
      res.status(200).json({ result: results });
