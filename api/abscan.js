export default async function handler(req, res) {
  ...
  try {
    if (action === 'rewards') {
      // Get internal txs FROM rewards contract, filter by wallet
      ...
      for (const tx of (data.result || [])) {
        if (
          tx.from?.toLowerCase() === REWARDS &&
          tx.to?.toLowerCase() === addrLow &&
          parseInt(tx.value) > 0
        ) {
          results.push({
      ...
      // Get internal txs FROM deposit contract, filter by wallet
      ...
      for (const tx of (data.result || [])) {
        if (
          ...
          tx.to?.toLowerCase() === DEPOSIT &&
          parseInt(tx.value) >= 6900000000000000
        ) {
          results.push({
            value: parseInt(tx.value) / 1e18,
            timestamp: new Date(parseInt(tx.timeStamp) * 1000).toISOString(),
